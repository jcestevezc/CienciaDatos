...
def elColombiano(root2,sql_digital_press):
    title = root.xpath("//meta[@property='og:title']")
    content = root.xpath("//article[@class='articulo fontChange no-lateral']")
    author = root.xpath("//meta[@name='author']")
    date = root.xpath("//div[@class='autor']//h6")
    medio = root.xpath("//meta[@property='og:site_name']")
    type_article = root.xpath("//meta[@property='og:type']/@content") 
    key_words = root.xpath("//meta[@name='keywords']/@content")
...
def clean(dictionary):
    ...
    register.title = str(register.title.values[0]).replace("[", "").replace("]", "")
    register.content = str(register.content.values[0]).replace("[", "").replace("]", "")
    register.author = str(register.author.values[0]).replace("[", "").replace("]", "")
    register.date = str(register.date.values[0]).replace("[", "").replace("]", "")
    register.medio = str(register.medio.values[0]).replace("[", "").replace("]", "")
    register.type_article = str(register.type_article.values[0]).replace("[", "").replace("]", "")
    register.key_words = str(register.key_words.values[0]).replace("[", "").replace("]", "")
    return register
